...
namespace CL_AD
{
    public class ADarchivos
    {
        ...
        public ENarchivos VERSION1_SP_EliminarImagenPorProducto(string id_producto, string url_imagen)
        {
            string cadenaConexion = micx.TraerMiConexion();
            ENarchivos ent = new ENarchivos();
            try
            {
                ...
                Parametro[1] = new SqlParameter("@url_imagen", SqlDbType.VarChar, 255);
                Parametro[1].Direction = ParameterDirection.Input;
                Parametro[1].Value = url_imagen;

                SqlHelper.ExecuteNonQuery(conexion, CommandType.StoredProcedure, "SP_EliminarImagen", Parametro);
        ...
        public List<ENImagenProducto> VERSION1_SP_ListarImagenesPorProducto(string idProducto, string rutaBaseImagenes)
        {
            var listaImagenes = new List<ENImagenProducto>();
            string cadenaConexion = micx.TraerMiConexion();
            ...
            try
            {
                ...
                {
            new SqlParameter("@id_producto", SqlDbType.Int) { Value = Convert.ToInt32(idProducto) }
                };

                using var reader = SqlHelper.ExecuteReader(conexion, CommandType.StoredProcedure, "VERSION1_SP_ListarImagenesPorProducto", parametros);

                while (reader.Read())
                {
                    var imagen = new ENImagenProducto
                    {
                        IdImagenProducto = Convert.ToInt32(reader["id_imagen_producto"]),
                        UrlImagen = DataUtil.ObjectToString(reader["url_imagen"]),
                        EsPrincipal = Convert.ToBoolean(reader["es_principal"])
                    };

                    if (!string.IsNullOrEmpty(imagen.UrlImagen))
                    {
                        var nombreArchivo = imagen.UrlImagen.Split("/");

                        if (nombreArchivo.Length >= 4)
                        {
                            var rutaImagen = Path.Combine(rutaBaseImagenes, nombreArchivo[1], nombreArchivo[2], nombreArchivo[3]);

                            if (System.IO.File.Exists(rutaImagen))
                            {
                                byte[] imageBytes = System.IO.File.ReadAllBytes(rutaImagen);
                                imagen.ImagenBase64 = Convert.ToBase64String(imageBytes);
                            }
                        }
                    }
