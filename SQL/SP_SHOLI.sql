...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 't_usuarios')
BEGIN
...
IF  EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'VERSION1_SP_guardarRutaPdfPorProducto')
BEGIN
EXEC('Drop  PROCEDURE VERSION1_SP_guardarRutaPdfPorProducto')
end
go
Create PROCEDURE VERSION1_SP_guardarRutaPdfPorProducto
@id_producto int,
@url_imagen varchar(500),
@id_usuario int
as
...
--/imagenes/productos/afeitadora-electrica_4.webp

/*
PROCEDIMIENTOS DE LISTADO
*/
IF EXISTS (SELECT NAME FROM sys.objects WHERE type = 'p' and name  = 'VERSION1_SP_ListarImagenesPorProducto' )
begin
	exec ('drop procedure VERSION1_SP_ListarImagenesPorProducto')
end
go
CREATE PROCEDURE VERSION1_SP_ListarImagenesPorProducto
    @id_producto INT
AS
BEGIN
    SELECT id_imagen_producto,url_imagen,es_principal
    FROM t_imagenes_productos
...
begin
	...
	where pro.estado = '1' and ma.estado = '1' and ca.estado = '1' and cp.estado = '1'
end
GO
...
IF  EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'VERSION1_SP_Listar_Unidades_Medida')
BEGIN
EXEC('Drop  PROCEDURE VERSION1_SP_Listar_Unidades_Medida')
end
go
Create PROCEDURE VERSION1_SP_Listar_Unidades_Medida
as
begin
	select um.id_unidad_medida, um.nombre, um.abreviatura from t_unidades_medidas as um where estado = '1'
end
GO
...
Login
*/
If exists (select name from sys.objects where type = 'p' and name = 'VERSION_SP_buscar_usuario')
begin
	exec('Drop PROCEDURE VERSION_SP_buscar_usuario')
end
go
Create PROCEDURE VERSION1_SP_buscar_usuario
@usuario varchar(255),
@clave varchar(255)
AS
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM t_usuarios WHERE usuario = @usuario AND clave = @clave)
    BEGIN 
        SELECT 
...
--Crud Para productos
IF EXISTS (SELECT NAME FROM sys.objects WHERE type = 'p' AND name = 'VERSION1_SP_listar_productos')
begin
exec('Drop PROCEDURE VERSION1_SP_listar_productos')
end
go
Create procedure VERSION1_SP_listar_productos
as
begin
	select pro.id_producto, 
	pro.codigo, 
	...
	ca.nombre as 'categoria', 
	ca.slug_categoria,
	tum.id_unidad_medida,
	tum.nombre as 'Unidad de medida'
	from t_productos as pro
	inner join t_marcas as ma on ma.id_marca = pro.id_marca
	inner join t_categorias_productos as cp on cp.id_producto = pro.id_producto
	inner join t_categorias as ca on ca.id_categoria = cp.id_categoria
	LEFT JOIN t_productos_unidades_medidas AS um ON um.id_producto = pro.id_producto AND um.estado = '1'
	LEFT JOIN t_unidades_medidas AS tum ON um.id_unidad_medida = tum.id_unidad_medida
	where pro.estado = '1' and ma.estado = '1' and ca.estado = '1' and cp.estado = '1'
	ORDER BY pro.nombre ASC
end
go

if EXISTS (select name from sys.objects where type = 'p' and name = 'SP_ListarProductoxID')
begin 
	exec('DROP PROCEDURE SP_ListarProductoxID')
end
go
Create procedure SP_ListarProductoxID
@id_producto int
as
begin
	select 
	...
	LEFT JOIN t_productos_unidades_medidas AS um ON um.id_producto = pro.id_producto AND um.estado = '1'
	LEFT JOIN t_unidades_medidas AS tum ON um.id_unidad_medida = tum.id_unidad_medida
	where pro.estado = '1' and ma.estado = '1' and ca.estado = '1' and cp.estado = '1' and pro.id_producto = @id_producto
end
go

update t_productos
set
urlimagen2 = '/imagenes/productos/afeitadora-electrica_2.webp'
where id_producto = 47

update t_imagenes_productos
set
es_principal =1
where id_imagen_producto=42


select * from t_productos order by nombre asc
select * from t_imagenes_productos

IF EXISTS (SELECT name FROM sys.objects WHERE type = 'p' AND name = 'SP_EditarProducto')
BEGIN 
    EXEC('DROP PROCEDURE SP_EditarProducto')
END
GO
...
AS
BEGIN
    SET XACT_ABORT ON; --aborto automático en errores severos

    DECLARE @result VARCHAR(3) = '1';
    ...
    BEGIN TRY
        ...
        SET
            ...
            fecha2 = dbo.dReturnDate(GETDATE()),
            usu2 = @Usu2,
			ficha = ISNULL(@ficha, ficha)
        WHERE id_producto = @Id_producto;

        IF @@ROWCOUNT = 0
        BEGIN
            RAISERROR('No se pudo actualizar el producto.', 16, 1);
        END

        -- Actualizar categoría
        IF @Id_categoria > 0
        BEGIN
            -- Desactivar otras categorías activas
            UPDATE t_categorias_productos
            SET estado = '2', fecha2 = dbo.dReturnDate(GETDATE()), usu2 = @Usu2
            WHERE id_producto = @Id_producto AND estado = '1';

            -- Activar categoría existente o insertarla si no existe
            IF EXISTS (
                SELECT 1 FROM t_categorias_productos
                WHERE id_producto = @Id_producto AND id_categoria = @Id_categoria
            )
            BEGIN
                UPDATE t_categorias_productos
                SET estado = '1', fecha2 = dbo.dReturnDate(GETDATE()), usu2 = @Usu2
                WHERE id_producto = @Id_producto AND id_categoria = @Id_categoria;
            END
            ELSE
            BEGIN
                INSERT INTO t_categorias_productos (id_categoria, id_producto, fecha1, usu1, estado)
                VALUES (@Id_categoria, @Id_producto, dbo.dReturnDate(GETDATE()), @Usu2, '1');
            END
        END
        ...
        IF @Id_unidad > 0
        BEGIN
            -- Desactivar otras unidades activas
            UPDATE t_productos_unidades_medidas
            SET estado = '2', fecha2 = dbo.dReturnDate(GETDATE()), usu2 = @Usu2
            WHERE id_producto = @Id_producto AND estado = '1';

            -- Activar unidad existente o insertarla si no existe
            IF EXISTS (
                SELECT 1 FROM t_productos_unidades_medidas
                WHERE id_producto = @Id_producto AND id_unidad_medida = @Id_unidad
            )
            BEGIN
                UPDATE t_productos_unidades_medidas
                SET estado = '1', fecha2 = dbo.dReturnDate(GETDATE()), usu2 = @Usu2
            ...
            ELSE
            BEGIN
                INSERT INTO t_productos_unidades_medidas (id_unidad_medida, id_producto, fecha1, usu1, estado)
                VALUES (@Id_unidad, @Id_producto, dbo.dReturnDate(GETDATE()), @Usu2, '1');
            END
        END

        -- Commit solo si la transacción está abierta
        IF @@TRANCOUNT > 0
            COMMIT;
        
        SELECT @result AS Resultado; -- Resultado exitoso
    END TRY
    BEGIN CATCH
...
IF EXISTS (SELECT name FROM sys.objects WHERE type = 'p' AND name = 'SP_EliminarImagen')
BEGIN 
    EXEC('DROP PROCEDURE SP_EliminarImagen')
END
GO
...
begin
	...
	estado = 2,
	es_principal = 0 
	where id_producto = @id_producto and url_imagen = @url_imagen
end
go

IF EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'VERSION1_SP_EditarImage')
BEGIN
    EXEC('DROP PROCEDURE VERSION1_SP_EditarImage')
END
GO

CREATE PROCEDURE VERSION1_SP_EditarImage
    @id_producto INT,
    @url_imagen VARCHAR(500),
    @es_principal BIT = 0,
    @es_principal2 BIT = 0,
...
BEGIN
    ...
    BEGIN
		if	@es_principal = 1 or @es_principal2 = 1
		begin
			UPDATE t_imagenes_productos
			SET 
				es_principal = 1,
    ...
    BEGIN
		if	@es_principal = 1 or @es_principal2 = 1
		begin
			INSERT INTO t_imagenes_productos (
				id_producto,
				url_imagen,
				es_principal,
				fecha1,
